...
const generateCustomerEmailHTML = (booking: any, emailType: string, cancellationToken?: string, refundAmount?: number): string => {
  ...
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Booking ${emailType.charAt(0).toUpperCase() + emailType.slice(1)}</title></head><body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f1f5f9; line-height: 1.6;"><p>Email content truncated for brevity</p></body></html>`;
};

const generateAdminDispatchEmailHTML = (booking: any): string => {
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>New Booking Alert</title></head><body><p>Admin email truncated for brevity</p></body></html>`;
};
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (isAdminEmail) {
      ...
      emailHTML = generateCustomerEmailHTML(booking, emailType);
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        message: emailSent 
          ...
          : `Email logged for ${recipientEmail}`,
